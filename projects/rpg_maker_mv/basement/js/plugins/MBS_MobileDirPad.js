...
(function ($) {
	...
	Scene_Base.prototype.showUserInterface = function() {
		this._dirPad.hide(); // DAVE we don't use the dpad
		this._aButton.show();
		this._cButton.show();
	};
    ...
    // normal non mobile case
		if (!(this.isMobileDevice() && Scene_Base.dirpad)) {
      Scene_Map_processMapTouch.apply(this, arguments);
    }
    // mobile case: do if it's not around the buttons
    else {
    if (TouchInput.isTriggered() || this._touchCount > 0) {
        if (TouchInput.isPressed()) {
            if (this._touchCount === 0 || this._touchCount >= 15) {
                var scene = this;
                var button = scene._aButton;
                var rect = new PIXI.Rectangle(button.x - button.width * button.anchor.x, button.y - button.height * button.anchor.y, button.width, button.height);
                // not action button
                if(!rect.contains(TouchInput.x, TouchInput.y)) {
                  button = scene._cButton;
                  rect = new PIXI.Rectangle(button.x - button.width * button.anchor.x, button.y - button.height * button.anchor.y, button.width, button.height);
                  // not cancel button
                  if(!rect.contains(TouchInput.x, TouchInput.y)) {
                    // accept map input
