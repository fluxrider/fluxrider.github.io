...
<xsl:template match="/">
	...
	<tr>
	 ...
	 <script>
	 function canvas_init_<xsl:value-of select="$game_id"/>() { 
		 window.alert("TEST canvas_init");
	 ...
	 <td><a id="canvas_link_{$game_id}"><canvas id="canvas_{$game_id}" width="162" height="100" style="border:1px solid #000000;" onload="window.alert('TEST load ' + canvas_init_{$game_id});"/></a></td>
