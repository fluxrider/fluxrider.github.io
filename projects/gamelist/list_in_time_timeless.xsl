...
<xsl:if test="timeless">
  ...
  <xsl:choose>
   <xsl:when test="/gamelist/shots/shot[@game=$game_id]"><td><a id="canvas_link_{$game_id}"><canvas id="canvas_{$game_id}" width="324" height="200" onload="{$canvas_init}"/></a></td></xsl:when>
